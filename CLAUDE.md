...
A second fixture source, `tools/real-block-gen` (vendored source; applies to a
local upstream openvm checkout — see its README), dumps a **real openvm guest
block** by tapping the SDK's per-segment `ProvingContext`, vs fixture-gen's
synthetic AIRs. Use it when per-stage profiling needs a representative input —
fixture-gen's one narrow AIR with tiny non-scaling interactions cannot rank
prover stages. Its interactions are **node-index references into the constraint
DAG** (real openvm interactions are expression-valued); the GKR input layer
evaluates them with `eval_interactions`, NOT the column-index form the synthetic
`DummyInteractionAir`s happen to use. `--ref-prove` also dumps the matching
`outputs/` by re-proving the tapped context with a recording transcript.
