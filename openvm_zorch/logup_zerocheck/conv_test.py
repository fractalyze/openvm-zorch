...
def _conv(a: list, b: list) -> list:
    ...
    out = [fnp.zeros((), EF) for _ in range(len(a) + len(b) - 1)]
...
def _ef_list(vals: list[int]) -> list:
    return [f_to_ef(fnp.array(v, F)) for v in vals]
...
class BatchedConvTest(absltest.TestCase):
    def test_matches_scalar_conv(self) -> None:
        ...
        kernel = fnp.stack(_ef_list(kernel_vals))
        rows = fnp.stack([fnp.stack(_ef_list(r)) for r in rows_vals])
    ...
    def test_rank1_input(self) -> None:
        ...
        row_vals = [4, 5, 6, 7]
        kernel = fnp.stack(_ef_list(kernel_vals))
        coeffs = fnp.stack(_ef_list(row_vals))
        got = _batched_conv(coeffs, kernel)
        self.assertEqual(got.shape, (len(row_vals) + len(kernel_vals) - 1,))
        want = fnp.stack(_conv(_ef_list(row_vals), _ef_list(kernel_vals)))
