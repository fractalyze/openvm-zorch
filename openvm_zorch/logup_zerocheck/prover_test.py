...
class ZerocheckByteMatchTest(absltest.TestCase):
    ...
    def _load_case(self):
        ...
        xi = [ef_from_limbs(fnp.array(row, fnp.uint32)) for row in xi_rows]
        beta = ef_from_limbs(
            fnp.array(np.load(_FIXTURE / "outputs" / "beta.npy"), fnp.uint32)
        )
    ...
    def test_stage3_matches(self) -> None:
        ...
        np.testing.assert_array_equal(
            _ef_limbs(fnp.stack(proof.univariate_round_coeffs)),
        ...
        np.testing.assert_array_equal(_ef_limbs(fnp.stack(proof.r)), want_r)
        ...
        t, nxt = t.sample(1)
        got = int(np.asarray(lax.bitcast_convert_type(nxt, F).astype(fnp.uint32))[0])
    ...
    def test_stage3_reuse_after_chain_rebuild(self) -> None:
        ...
        np.testing.assert_array_equal(
            _ef_limbs(fnp.stack(first.r)), _ef_limbs(fnp.stack(second.r))
        )
        np.testing.assert_array_equal(
            _ef_limbs(fnp.stack(first.sumcheck_round_polys)),
            _ef_limbs(fnp.stack(second.sumcheck_round_polys)),
        )
        np.testing.assert_array_equal(
            _ef_limbs(fnp.stack(first.univariate_round_coeffs)),
            _ef_limbs(fnp.stack(second.univariate_round_coeffs)),
        )
