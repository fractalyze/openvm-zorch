...
def _single_col_traces(*cols: list[int]) -> list[fnp.ndarray]:
    return [fnp.array(c, dtype=F).reshape(-1, 1) for c in cols]
...
class StackingTest(absltest.TestCase):
    def test_manual_0(self) -> None:
        ...
        expected = fnp.array([1, 2, 3, 4, 5, 6, 7, 0], dtype=F).reshape(2, 4).T
        self.assertTrue(bool(fnp.array_equal(mat, expected)))
    ...
    def test_manual_strided_0(self) -> None:
        ...
        expected = (
            fnp.array([1, 2, 3, 4, 5, 0, 6, 0, 7, 0, 0, 0], dtype=F).reshape(3, 4).T
        )
        self.assertTrue(bool(fnp.array_equal(mat, expected)))
